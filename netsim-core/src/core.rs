...
impl DnsEntries {
    fn new(prefix: &str) -> Result<Self> {
        let hosts_dir = std::env::temp_dir().join(format!("netsim-{prefix}-hosts"));
    ...
    pub fn hosts_path_for(&self, device_id: NodeId) -> PathBuf {
    ...
    pub fn ensure_hosts_file(&self, device_id: NodeId) -> Result<()> {
    ...
    pub fn write_hosts_file(&self, device_id: NodeId) -> Result<()> {
        let path = self.hosts_path_for(device_id);
        let mut buf = String::from("# generated by netsim\n127.0.0.1\tlocalhost\n::1\tlocalhost\n");
    ...
    pub fn write_all_hosts_files(&self, device_ids: &[NodeId]) -> Result<()> {
    ...
    pub fn write_resolv_conf(&self) -> Result<()> {
        ...
        let content = match self.nameserver {
            Some(ip) => format!("# generated by netsim\nnameserver {ip}\n"),
            None => "# generated by netsim\n".to_string(),
        };
    ...
    pub fn resolve(&self, device_id: Option<NodeId>, name: &str) -> Option<IpAddr> {
