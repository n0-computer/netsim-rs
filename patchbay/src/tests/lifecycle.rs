...
/// Calling `TestGuard::ok()` sets the test status to SUCCESS.
#[tokio::test(flavor = "current_thread")]
#[traced_test]
async fn test_guard_ok_marks_success() -> Result<()> {
    use std::sync::atomic::Ordering;

    let lab = Lab::new().await?;
    let guard = lab.test_guard();

    // Before .ok(), status should still be UNKNOWN.
    assert_eq!(
        lab.inner.test_status.load(Ordering::Acquire),
        writer::STATUS_UNKNOWN,
        "status should be UNKNOWN before .ok()"
    );
    ...
    assert_eq!(
        ...
        "status should be SUCCESS after .ok()"
    );
    Ok(())
}

/// Dropping a `TestGuard` without calling `.ok()` sets the test status to FAILED.
#[tokio::test(flavor = "current_thread")]
#[traced_test]
async fn test_guard_drop_marks_failed() -> Result<()> {
    use std::sync::atomic::Ordering;
    ...
    // Drop without calling .ok() — simulates a panic or early `?` return.
    drop(guard);

    assert_eq!(
        lab.inner.test_status.load(Ordering::Acquire),
        writer::STATUS_FAILED,
        "status should be FAILED after drop without .ok()"
    );
    Ok(())
}

/// A second `TestGuard` on the same lab sees the status left by the first.
/// Calling `.ok()` on the second guard overwrites to SUCCESS.
#[tokio::test(flavor = "current_thread")]
#[traced_test]
async fn test_guard_second_guard_overwrites() -> Result<()> {
    use std::sync::atomic::Ordering;

    let lab = Lab::new().await?;

    // First guard dropped without .ok() → FAILED.
    let g1 = lab.test_guard();
    drop(g1);
    assert_eq!(
        lab.inner.test_status.load(Ordering::Acquire),
        writer::STATUS_FAILED,
    ...
    // Second guard calls .ok() → SUCCESS.
    let g2 = lab.test_guard();
    g2.ok();
    assert_eq!(
        lab.inner.test_status.load(Ordering::Acquire),
        writer::STATUS_SUCCESS,
