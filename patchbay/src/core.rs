...
/// A shared `resolv.conf` lives at `<hosts_dir>/resolv.conf`. Files are bind-mounted
/// into worker threads at startup. Lab-wide DNS records live in [`DnsServer`] instead.
...
impl DnsEntries {
    ...
    /// Writes (or rewrites) the hosts file for a single device (device-local entries only).
    ...
    /// Resolves a name from per-device entries. Returns the first match.
    pub(crate) fn resolve(&self, device_id: NodeId, name: &str) -> Option<IpAddr> {
        if let Some(entries) = self.per_device.get(&device_id) {
            for (n, ip) in entries {
                if n == name {
                    return Some(*ip);
...
pub(crate) struct LabInner {
    ...
    pub dns_server: tokio::sync::OnceCell<crate::dns_server::DnsServer>,
