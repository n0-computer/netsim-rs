...
pub(crate) struct DnsOverlayDir {
    /// Optional IPv4 nameserver for `/etc/resolv.conf` overlay.
    pub nameserver: Option<IpAddr>,
    /// Optional IPv6 nameserver for `/etc/resolv.conf` overlay.
    pub nameserver_v6: Option<IpAddr>,
...
impl DnsOverlayDir {
    ...
    pub(crate) fn write_resolv_conf(&self) -> Result<()> {
        ...
        if let Some(ip) = self.nameserver {
            content.push_str(&format!("nameserver {ip}\n"));
        }
        if let Some(ip) = self.nameserver_v6 {
            content.push_str(&format!("nameserver {ip}\n"));
        }
