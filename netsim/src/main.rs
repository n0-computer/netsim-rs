...
enum Command {
    ...
    Run {
        ...
        /// Clone and run from the repo's `netsim.toml` project config.
        #[arg(long)]
        repo: Option<String>,
        ...
        /// Clone repo from `netsim.toml` and run sims from the clone root.
...
fn resolve_run_spec(
    ...
    if clone_mode {
        ...
        let config_path = if sims.is_empty() {
            ...
            find_netsim_toml(&cwd).ok_or_else(|| {
                anyhow!(
                    "no netsim.toml found from {} upward (required by --clone)",
                    cwd.display()
                )
            })?
        } else {
            ...
            find_netsim_toml(&sims[0]).ok_or_else(|| {
                anyhow!(
                    "no netsim.toml found from {} upward (required by --clone)",
                    sims[0].display()
                )
        ...
        let cfg = load_netsim_config(&config_path)?;
        let repo_url = cfg.repo.clone().ok_or_else(|| {
            anyhow!(
                "netsim.toml at {} is missing 'repo' (required by --clone)",
                config_path.display()
            )
        })?;
    ...
    if repo.is_none() && repo_ref.is_some() {
        bail!("--ref requires --repo (or use --clone with netsim.toml repo)");
    }
    ...
    if let Some(repo_url) = repo {
        let checkout = clone_or_update_repo(work_dir, &repo_url, repo_ref.as_deref())?;
        let config_path = find_netsim_toml(&checkout).ok_or_else(|| {
            anyhow!(
                "no netsim.toml found under cloned repo root {}",
                checkout.display()
            )
        })?;
        let cfg = load_netsim_config(&config_path)?;
    ...
    let cwd = std::env::current_dir().context("resolve current directory")?;
    let config_path = find_netsim_toml(&cwd).ok_or_else(|| {
        anyhow!(
            "no sims provided and no netsim.toml found from {} upward",
            cwd.display()
        )
    })?;
    let cfg = load_netsim_config(&config_path)?;
...
fn load_netsim_config(config_path: &std::path::Path) -> Result<NetsimProjectConfig> {
    let text = std::fs::read_to_string(config_path)
        .with_context(|| format!("read {}", config_path.display()))?;
    let cfg: NetsimProjectConfig =
        toml::from_str(&text).with_context(|| format!("parse {}", config_path.display()))?;
    Ok(cfg)
}

fn run_spec_from_config(cfg: &NetsimProjectConfig, sim_root: &std::path::Path) -> Result<RunSpec> {
...
fn find_netsim_toml(start: &std::path::Path) -> Option<PathBuf> {
    let mut dir = start.to_path_buf();
    if dir.is_file() {
        dir = dir.parent()?.to_path_buf();
    }
    loop {
        let candidate = dir.join("netsim.toml");
...
fn perform_cleanup(_prefixes: &[String]) -> Result<()> {
    tracing::info!(
        "netsim cleanup: fd-based namespaces are automatically cleaned up on drop; nothing to do"
    );
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn find_netsim_toml_walks_parents() {
        let root = std::env::temp_dir().join(format!(
            "netsim-config-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
        ...
        let config = write_temp_file(
            &root,
            "proj/netsim.toml",
            "repo = \"https://example.com/repo.git\"\nsimulations = \"netsim/sims/**\"\n",
        );
        let nested = root.join("proj").join("a").join("b");
        std::fs::create_dir_all(&nested).expect("create nested");
        let found = find_netsim_toml(&nested).expect("find config");
    ...
    fn expand_sim_glob_ignores_non_toml() {
        ...
        write_temp_file(&root, "proj/netsim/sims/a.toml", "x");
        write_temp_file(&root, "proj/netsim/sims/b.txt", "x");
        let sims = expand_sim_glob(&root.join("proj"), "netsim/sims/**").expect("expand glob");
