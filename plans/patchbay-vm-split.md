# patchbay-vm Standalone VM Runner Plan

## TODO

- [x] Write plan
- [x] Add standalone `crates/patchbay-vm` bin crate and workspace wiring
- [x] Migrate VM lifecycle/mount logic from `src/vm.rs` into `patchbay-vm`
- [x] Add `patchbay-vm run` with `--netsim-version` (latest, release tag, `git:<ref>`, `path:`)
- [x] Add `patchbay-vm test` VM test execution (host build/discover, guest execute, summary)
- [x] Replace Makefile VM tasks with `cargo run -p patchbay-vm -- ...`
- [x] Remove `qemu-vm.sh` file from repository
- [x] Remove `netsim run-vm` path from `patchbay` binary (deferred by request)
- [ ] Final review

## Goal
Replace shell-based VM orchestration (`qemu-vm.sh`) with a standalone workspace binary crate `patchbay-vm` that supports both VM sim runs and VM test execution (`cargo make test-vm` parity), with no Rust crate dependency between `patchbay` and `patchbay-vm`.

## Hard Requirements
1. `patchbay-vm` is a binary crate in this workspace.
2. Preferred: no Rust dependency edges between `patchbay` and `patchbay-vm`.
   - Transitional allowance: `patchbay-vm -> netsim` dependency is acceptable now to share build/staging logic; can be feature-gated or removed later.
3. `patchbay-vm` can:
   - run sims in VM using a selected `patchbay` source (`latest` release, release tag, `git:<ref>`, or `path:<local-binary>`)
...
1. `patchbay-vm run [OPTIONS] <SIM_OR_DIR>...`
   - Purpose: VM equivalent of current `netsim run-vm`.
   - Args:
     - `--work-dir <PATH>` default `.patchbay-work`
...
2. `patchbay-vm test [OPTIONS] [-- <CARGO_TEST_ARGS...>]`
...
- Behavior:
  - Resolve `patchbay` by `--netsim-version`:
    - `latest` / release tag -> download GH release artifact
    - `git:<ref>` -> host-build `patchbay` for musl from workspace
...
## Test Flow Design (`patchbay-vm test`)
...
### `patchbay-vm test` command contract
...
1. Replace shell-wrapper VM tasks:
   - `setup-vm` -> `cargo run -p patchbay-vm -- up`
   - `run-vm` -> `cargo run -p patchbay-vm -- run ...`
   - `test-vm` -> `cargo run -p patchbay-vm -- test ...`
   - `vm-status` -> `cargo run -p patchbay-vm -- status`
   - `vm-down` -> `cargo run -p patchbay-vm -- down`
2. Remove dependency on `qemu-vm.sh` in tasks.
3. After validation, remove `qemu-vm.sh` file.

## netsim CLI Changes

1. Remove `run-vm` command from `patchbay` binary (strict ownership split).
2. `netsim cleanup` no longer stops VM; VM lifecycle is `patchbay-vm down/cleanup`.

## Implementation Steps

1. Workspace setup
   - add `crates/patchbay-vm` bin crate.
   - update root workspace members.
2. VM logic move
   - migrate `src/vm.rs` logic into `crates/patchbay-vm/src/vm.rs` (single module style).
   - keep code close to existing literal command flow; avoid structural refactors.
3. GH `patchbay` download runner path
   - keep robust staging and guest invocation (helper functions may live in `util.rs`).
4. Add `patchbay-vm test` command
   - implement build/discover/execute/summarize test flow.
5. Rewire Makefile tasks
   - replace shell script calls with `patchbay-vm`.
6. Remove old integration
   - remove `src/vm.rs` and `patchbay` `run-vm`.
7. Docs + plan updates
...
8. Validation
   ...
   - `cargo test`
   - smoke:
     - `cargo run -p patchbay-vm -- up`
     - `cargo run -p patchbay-vm -- run ...`
     - `cargo run -p patchbay-vm -- test`
     - `cargo run -p patchbay-vm -- down`

## Shared Code Note
- For initial delivery, allow `patchbay-vm` to reuse build/staging helpers from `patchbay` if it materially reduces duplicate logic.
...
2. `patchbay-vm run` executes sims in guest using downloaded release `patchbay`.
3. `patchbay-vm test` executes compiled tests in guest and returns correct aggregate status.
4. No Rust dependency between `patchbay` and `patchbay-vm`.
