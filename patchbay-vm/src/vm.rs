...
/// `patchbay-vm down` entrypoint.
pub fn down_cmd() -> Result<()> {
    stop_vm_if_running()
}

/// `patchbay-vm cleanup` entrypoint.
...
fn run_in_guest(vm: &VmConfig, args: &RunVmArgs) -> Result<()> {
    let guest_exe = ensure_guest_runner_binary(vm, &args.patchbay_version)?;
...
fn ensure_guest_runner_binary(vm: &VmConfig, version: &str) -> Result<String> {
    ...
    let staged_dir = vm.work_dir.join(".patchbay-bin");
...
/// Locate the UEFI firmware blob needed by `qemu-system-aarch64`.
///
/// Searches relative to the QEMU binary first (handles Nix and Homebrew
/// layouts), then falls back to common system paths.
...
fn render_cloud_init(vm: &VmConfig) -> Result<()> {
    ...
    // Match by driver rather than interface name: x86 guests use eth0, arm64
    // guests use enp0s1 (or similar), but both use virtio_net.
    std::fs::write(
        vm.network_cfg(),
        "version: 2\nethernets:\n  id0:\n    match:\n      driver: virtio_net\n    dhcp4: true\n",
    )
...
fn start_vm(vm: &mut VmConfig) -> Result<()> {
    ...
    // aarch64 requires an explicit machine type and UEFI firmware.
    if is_aarch64 {
        qemu.arg("-M").arg("virt");
